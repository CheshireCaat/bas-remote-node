...
module.exports = class EngineService extends EventEmitter {
   ...
   */
  async initialize() {
    await axios.get(`${URL}/scripts/${this.options.scriptName}/properties`)
      .then(({ data }) => {
        if (!data.success) {
  ...
  async _downloadExecutable(zipPath, zipName, urlName) {
    const loader = new Loader().on('log', (message) => {
      this.emit('log', message);
    });

    const buffer = await loader.get(`${URL}/distr/${urlName}/${basename(this.zipDir)}/${zipName}.zip`);
  ...
  async _extractExecutable(zipPath) {
    await extract(zipPath, { dir: this.exeDir });
  }
