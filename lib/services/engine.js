...
module.exports = class EngineService {
    ...
    constructor(options) {
        ...
        this._request = request.defaults({ baseUrl: 'https://bablosoft.com' });
    }
     ...
     */
    async initialize() {
        return this._request(`/scripts/${this.options.scriptName}/properties`, { json: true })
            .then((data) => {
                if (!data.success) {
    ...
    async _downloadExecutable(zipPath, zipName, urlName) {
        return this._request(`/distr/${urlName}/${basename(this.zipDir)}/${zipName}.zip`, { encoding: null })
            .then((data) => {
    ...
    _getLockPath(dir) {
        return join(dir || this.exeDir, '.lock');
    }
