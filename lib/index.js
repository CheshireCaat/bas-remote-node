...
module.exports = class BasRemoteClient extends EventEmitter {
  /**
  ...
  constructor(options) {
    ...
    this._socket.on('messageReceived', (message) => {
    ...
    ['messageReceived', 'messageSend', 'log'].forEach((event) => {
      if (event === 'log') {
        this._engine.on(event, (...args) => this.emit(event, ...args));
      } else {
  ...
  *   - `log` - invoked when a new log message appears.
  *
