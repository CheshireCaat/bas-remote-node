const log = require('debug')('bas-remote-node:loader');
...
module.exports = {
  async get(url) {
    ...
    try {
      log(`Download metadata from the ${url}.meta.json`);
      ...
      if (typeof data === 'object' && ['TotalSize', 'Checksum', 'Chunks', 'Url'].every((key) => key in data)) {
        ...
        strict = true;
        log('Valid metadata - enable strict mode');
      } else {
        strict = false;
        log('Wrong metadata - disable strict mode');
      }
    } catch (err) {
    ...
    if (!strict) {
      try {
        ...
        if (headers['content-range']) {
          ...
          if (size) {
            log(`Obtained file size - ${(meta.size = size)}`);
          } else {
  ...
  async download(meta, strict) {
    ...
    while (offset < meta.size) {
      ...
      // }, CHECK_INTERVAL);
      try {
        log(`Start download chunk - ${offset}`);
        ...
        log(`Error during file chunk download - ${err.message}`);
        log(`Sleep for ${RETRY_SLEEP} milliseconds and repeat`);
